(function() {
    ...
    // BraveFox Enhancer v25.4.0 - Images/WebP-AVIF fallback helper
    // Goal:
    ...
    async function fetchImageAsDataUrlViaBackground(url) {
        ...
        const response = await runtimeSendMessage({
            type: 'BRAVEFOX_ANTIWEBP_FETCH_IMAGE_AS_DATA_URL',
            url: sourceUrl
        });

        if (!response || !response.ok || !response.dataUrl) {
            devLog('Background image fetch failed:', response && (response.error || response.status || response.contentType));
            return null;
        }
