(function () {
    ...
    const regexKeywordsToHide = [
        ...
        /pillu/i, /perse/i, /pylly/i, /peppu/i, /pimppi/i, /pinppi/i, /\bPeba\b/i, /\bBeba\b/i, /\bBabe\b/i, /\bBepa\b/i, /\bAnaali\b/i, /\bSeksi\b/i, /picture app/i, /edit app/i, /pic app/i, /photo app/i, /syvavaarennos/i, /Perry WWE/i,
        /application/i, /sukupuoliyhteys/i, /penetraatio/i, /penetration/i, /vaatepoisto/i, /vaatteidenpoisto/i, /poista vaatteet/i, /(?:poista|poisto|poistaminen)[ -]?(?:vaatteet|vaatteiden)/i, /\bAnus\b/i, /sexuaali/i, /\bAnal\b/i, /5yvä/i,
	...
	/\bRembg\b/i, /\bRem bg\b/i, /\bDel bg\b/i, /\bDelbg\b/i, /delet bg/i, /eras bg/i, /delet bg/i, /erase bg/i, /erasing bg/i, /bg delet/i, /bg erasing/i, /bg erase/i, /Blend face/, /Blendface/, /morphi/, /Blender face/, 
	...
	/removebg/i, /remove bg/i, /remov bg/i, /removal bg/i, /ia onl/i, /removebg/i,  /removalbg/i, /rembg/i, /rem background/i, /del background/i, /eras background/i, /erase background/i, /erasing background/i, 
	/\bIA\b/i,/\bIas\b/i, /\b-Ia\b/i, /\bIa-\b/i, /background eras/i, /background del/i, /background rem/i, /background off/i, /off background/i, /background out/i, /out background/i, /removbg/i,
    ];
	...
	"Celeste Bonin", "Ariane Andrew", "Brianna Monique Garcia", "Stephanie Nicole Garcia", "CJ Perry", "Lana Rusev", "Pamela Martinez", "Ashley Sebera", "Ayesha Raymond", "Marti Belle", "Alisha Edwards", 
	"Nicole Garcia", "Nikki Garcia", "Wrestling babe", "Divas hot", "WWE sexy", "spicy site", "for fans", "VIP pic", "premium content", "sussy pic", "after dark", "NSFL", "artistic nude", "tasteful nude", 
	"uncensored version", "sus content", "sus site", "alt site", "runwayml", "replicate.ai", "huggingface", "hugging face", "cloth remover", "AI eraser", "Magic Editor", "magicstudio", "cleanup.pictures", 
	"app123", "modapk", "apkmod", "tool hub", "tools hub", "alaston", "alasti", "vaatteeton", "paljas", "seksikäs", "pimppi", "vittu", "tissit", "nänni", "sukupuolielin", "paljain", "seksisivu", "alastomuus", 
	...
	/apteekki/i, /market/i, /k-market/i, /s-market/i, /marketti/i, /kauppa/i, /kauppatori/i, 
    ];
    ...
    // Selectors for Google search bar elements to never hide or delete
    ...
    // Function to check if an element matches any of the protected selectors
    const isProtectedElement = (element) => {
        return protectedSelectors.some(selector => element.matches(selector));
    };
    ...
    // Function to check if a query contains any forbidden keywords or phrases
    const containsForbiddenKeywords = (query) => {
        return regexKeywordsToHide.some(keyword => keyword.test(query)) ||
               stringKeywordsToHide.some(keyword => query.toLowerCase().includes(keyword.toLowerCase()));
    };
    ...
    const handleForbiddenSearchRedirection = () => {
        ...
        console.log(`Checking query for forbidden search: ${query}`);
        if (containsForbiddenKeywords(query) && !isUrlAllowed(window.location.href)) {
            console.log(`Redirecting to Google home due to forbidden search: ${query}`);
            window.location.replace('https://www.google.com');
    ...
    const checkUrlForForbiddenKeywords = () => {
        ...
        if (regexKeywordsToHide.some(keyword => keyword.test(url)) || stringKeywordsToHide.some(keyword => url.includes(keyword.toLowerCase()))) {
            console.log(`Redirecting to Google home due to forbidden URL: ${url}`);
            window.location.replace('https://www.google.com');
            return true;
        }
        return false;
    ...
    // Early exit if forbidden keyword is detected in the initial URL
    if (checkUrlForForbiddenKeywords()) {
        return;
    }

    // Function to clean the URL by keeping only 'q' and removing all other query parameters
    const cleanGoogleUrl = () => {
        const url = new URL(window.location.href);

        // If the URL is from a Google domain, clean it
        if (/^.*\.google\..*/.test(window.location.hostname)) {
            const params = url.searchParams;

            // Remove all parameters except for 'q'
            Array.from(params.keys()).forEach(param => {
                if (containsForbiddenKeywords(param)) {
                    console.log(`Redirecting to Google home due to forbidden query parameter: ${param}`);
                    window.location.replace('https://www.google.com');
                    return;
                }
                if (param !== 'q') {
            ...
            // If 'q' exists, clean the URL and update it
            if (params.has('q')) {
                const cleanedUrl = `${window.location.origin}${window.location.pathname}?${params.toString()}`;
                if (cleanedUrl !== window.location.href) {
                    console.log(`Updating URL to: ${cleanedUrl}`);
                    // Replace the URL immediately without redirecting
    ...
    const interceptSearchFormSubmit = () => {
        ...
        if (searchForm) {
            searchForm.addEventListener('submit', (event) => {
                ...
                console.log(`Form submission detected with query: ${query}`);
                // Check if the query contains any keywords to hide
                if (containsForbiddenKeywords(query) && !isUrlAllowed(window.location.href)) {
                    console.log(`Redirecting to Google home due to forbidden search: ${query}`);
    ...
    const interceptSearchInputChanges = () => {
        ...
        if (searchInput) {
            searchInput.addEventListener('input', () => {
                ...
                console.log(`Search input change detected with query: ${query}`);
                // Check if the query contains any keywords to hide
                if (containsForbiddenKeywords(query) && !isUrlAllowed(window.location.href)) {
                    console.log(`Redirecting to Google home due to forbidden search: ${query}`);
                    window.location.replace('https://www.google.com');
    ...
    const blockUrls = () => {
        ...
        for (let link of links) {
            ...
            if (!allowedWords.some(word => word.test(link.href)) && (blockKeywordsPattern.test(link.href) || urlPatternsToHide.some(pattern => pattern.test(link.href)))) {
                console.log(`Blocking URL: ${link.href}`);
                link.remove(); // Remove the link
            }
        }
    };

    // Function to block elements based on CSS selectors
    const blockElementsBySelectors = () => {
    const selectorsToHide = [
            'span.gL9Hy',
            '.spell_orig',
            '.KDCVqf.card-section.p64x9c',
            '#oFNiHe',
            '#taw',
            '.QRYxYe',
            '.NNMgCf',
        ...
        selectorsToHide.forEach(selector => {
            ...
            elements.forEach(element => {
                if (!allowedWords.some(word => word.test(element.textContent)) && !isProtectedElement(element)) {
                    console.log(`Blocking element matching selector: ${selector}`);
                    element.remove();
                }
            });
        });
    ...
    // Function to block elements containing specific phrases
    const blockElementsByPhrases = () => {
        const phrasesToHide = [
        ...
        elements.forEach(element => {
            // Ensure we do not block the Google search bar and essential elements
            if (isProtectedElement(element)) {
                return; // Skip blocking these essential elements
            }
            const textContent = element.textContent || '';
            if (phrasesToHide.some(phrase => phrase.test(textContent)) && !allowedWords.some(word => word.test(textContent))) {
                console.log(`Blocking element containing phrase: ${textContent}`);
                element.remove();
            }
        });
    };

    // Function to block results based on exact keywords and selectors
    const blockResults = () => {
        ...
        results.forEach(result => {
            ...
            console.log(`Checking result: ${resultUrl}`);
            if ((!allowedWords.some(word => word.test(resultText)) && blockKeywordsPattern.test(resultText)) || blockKeywordsPattern.test(resultUrl)) {
                if (!allowedWords.some(word => word.test(resultUrl)) && !isUrlAllowed(resultUrl) && (!link.href.includes('github.com') || blockKeywordsPattern.test(resultUrl))) {
                    console.log(`Blocking result: ${resultText} or URL: ${resultUrl}`);
                    result.remove();
                }
            }

            if (urlPatternsToHide.some(pattern => pattern.test(resultUrl))) {
                console.log(`Blocking URL: ${resultUrl}`);
                result.remove();
            }
        });

        // Handle Image Search results
        const imageResults = document.querySelectorAll('img, div[jsname="N9Xkfe"], div[jsname="pKbz0b"]');
        imageResults.forEach(image => {
            const altText = image.alt || image.innerText || "";
            const link = image.closest('a');
            const imageUrl = link ? link.href : '';

            console.log(`Checking image result: ${imageUrl}`);
            if ((!allowedWords.some(word => word.test(altText.toLowerCase())) && blockKeywordsPattern.test(altText.toLowerCase())) || blockKeywordsPattern.test(imageUrl)) {
                if (!allowedWords.some(word => word.test(imageUrl)) && !isUrlAllowed(imageUrl) && !urlPatternsToHide.some(pattern => pattern.test(imageUrl))) {
                    console.log(`Blocking image result: ${altText} or URL: ${imageUrl}`);
                    image.closest("div").remove();
                }
            }

            if (urlPatternsToHide.some(pattern => pattern.test(imageUrl))) {
                console.log(`Blocking image URL: ${imageUrl}`);
                image.closest("div").remove();
            }
        });

        // Block additional elements by selectors
        blockElementsBySelectors();
        // Block elements by phrases
        blockElementsByPhrases();
    };

    // Monitor for banned terms in the body text every 250ms
    const monitorBannedTerms = () => {
        setInterval(() => {
            const elements = document.querySelectorAll('body, body *');
            elements.forEach(element => {
                if (element.textContent && !allowedWords.some(word => word.test(element.textContent)) && blockKeywordsPattern.test(element.textContent)) {
    ...
    // Function to monitor selectors and redirect if banned words are found
    const monitorSelectorsAndRedirect = () => {
        ...
        selectors.forEach(selector => {
            ...
            elements.forEach(element => {
                const textContent = element.textContent || '';
                if (blockKeywordsPattern.test(textContent) || stringKeywordsToHide.some(keyword => textContent.toLowerCase().includes(keyword.toLowerCase()))) {
                    console.log(`Redirecting to Google home due to banned content in selector: ${selector}`);
                    window.location.replace('https://www.google.com');
                }
            });
        });
    ...
    const init = () => {
        enforceSafeSearch(); // Enforce SafeSearch passively
        interceptSearchInputChanges(); // Intercept search input changes
        interceptSearchFormSubmit(); // Intercept form submission

        // Call blockUrls, blockElementsBySelectors, monitorSelectorsAndRedirect, and blockResults immediately to hide elements as soon as possible
        blockUrls();
        blockElementsBySelectors();
        blockResults();
        ...
        const observer = new MutationObserver(() => {
            blockResults(); // Block results on updates
            blockUrls(); // Block URLs on updates
            blockElementsBySelectors(); // Block elements by selectors on updates
            monitorSelectorsAndRedirect(); // Monitor selectors and redirect on updates
        });
        ...
        // Observe the body for URL changes
        const bodyObserver = new MutationObserver(() => {
            cleanGoogleUrl(); // Clean the URL if any significant changes happen on the page
        });
    ...
    // Perform an immediate check for forbidden search on script load
    if (!handleForbiddenSearchRedirection() && !checkUrlForForbiddenKeywords()) {
        // Call the init function as soon as the DOM content is loaded
