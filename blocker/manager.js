...
function isLockError(message) {
  return /BraveFox Blocker is locked|management request denied|management is unavailable|request denied/i.test(String(message || ''));
}
...
function message(payload, { redirectOnLock = true } = {}) {
  return new Promise((resolve, reject) => {
    chrome.runtime.sendMessage(payload, response => {
      ...
      const errorText = runtimeError?.message || (!response?.ok ? (response?.error || 'BraveFox Blocker request failed.') : '');
      if (errorText) {
        if (redirectOnLock && isLockError(errorText)) secureLockout(true);
        reject(new Error(errorText || 'BraveFox Blocker request failed.'));
        return;
      }
      resolve(response);
    });
...
start().catch(error => {
  ...
  failure.textContent = `BraveFox Blocker failed to open: ${error.message}`;
