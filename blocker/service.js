...
async function authenticatePopupAndOpen(message, sender) {
  if (!senderIsLauncher(sender)) throw new Error('Blocker launcher request denied.');
  if (isIncognitoSender(sender)) throw new Error('Management is unavailable in private browsing.');

  const [activeTab] = await browser.tabs.query({ active: true, currentWindow: true });
  if (activeTab?.incognito) throw new Error('Management is unavailable in private browsing.');
  ...
  if (tab.incognito) {
    ...
    throw new Error('Management is unavailable in private browsing.');
  }
  try {
