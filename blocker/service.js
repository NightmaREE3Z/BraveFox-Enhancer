...
const NATIVE_LOG_SOURCE = chrome.runtime.getManifest().name === 'BraveFox Blocker'
  ? 'BraveFox Blocker'
  : 'BraveFox Blocker (BFE)';
...
async function requireManagerAccess(sender) {
  ...
  if (!(await isManagerTabUnlocked(tabId))) throw new Error('BraveFox Blocker is locked.');
  return tabId;
}
...
async function evaluateNavigation(tabId, url, title = '') {
  ...
  try {
    ...
    console.warn('[BraveFox Blocker] Redirect failed:', error);
  } finally {
  ...
  (async () => {
    switch (message.type) {
      ...
      case MESSAGE.openManager:
        throw new Error('Enter the popup password to open BraveFox Blocker.');
      default:
        throw new Error('Unknown BraveFox Blocker message.');
    }
...
void loadDataset({ force: true }).catch(error => console.warn('[BraveFox Blocker] Initial dataset load failed:', error));
console.log(`[BraveFox Blocker] Module ${BLOCKER_VERSION} initialized with resilient manager re-authentication, password-gated Settings, ordered persistent lists, dual redirects, manual profile sync, and native redirect logging.`);
