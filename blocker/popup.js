  ...
  updateButton.addEventListener('click', async () => {
    ...
    try {
      ...
      if (resultStatus === 'update_available') {
        ...
        showUpdateStatus(
          `Update${nextVersion} found. Chromium will install it automatically when ready.`,
          'success'
        );
      ...
      if (resultStatus === 'throttled') {
        showUpdateStatus('The browser throttled the update check. Try again later.', 'error');
      ...
      showUpdateStatus(updateError?.message || 'The browser could not complete the update check.', 'error');
      updateButton.textContent = 'Check again';
    } finally {
