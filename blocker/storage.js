...
async function writeSyncDataset(snapshot) {
  ...
  // Verify that the browser accepted the metadata and every current chunk.
  ...
  if (!verified || verified.revision !== clean.revision ||
    ...
    throw new Error('Sync storage verification failed after saving the blocklists.');
  }
...
export async function saveDataset({ terms, links, profile = '' }) {
  ...
  // Local mirror is committed first. A browser restart can therefore never
